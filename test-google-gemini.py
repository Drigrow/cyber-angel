...
openai.api_base = "https://gemini.0071126.xyz/v1"
# My proxy url, you can change it yourself
# attention: my proxy have convert gemini api requests into openai format, using url yourself may have comaibiliy issues.

# Initialize Porcupine with your custom wake word model
porcupine = pvporcupine.create(
    access_key='bbLJ3L5vU0ftQkswvWv8FsnmCUmygBz2psAKDECbKz6wx907/aYgJQ==',
...
model = whisper.load_model("small")  # Use "base" model; smaller models like "tiny" are faster
...
def estimate_background_noise(stream):
    ...
    for _ in range(int(RATE / FRAME_SIZE)):  # Approx. 1 second of audio
        ...
        rms = np.sqrt(max(np.mean(np.square(audio_data)), 0))  # Prevent negative values
...
def record_audio_with_silence_detection(stream, output_filename="output.wav"):
    ...
    silence_duration = 0
    silence_threshold = estimate_background_noise(stream)
